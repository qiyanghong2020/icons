---
name: icons
description: Find, preview, and download editable SVG icons for biomedical and scientific figures from BioIcons, Iconify, and optional Svg/icons CLI. Use when Codex needs icon assets for genes, DNA/RNA, cells, antibodies, immune cells, cancer, mutations, viruses, laboratory equipment, pathways, mechanisms, thesis figures, papers, posters, presentations, or UI diagrams, especially when the user wants reusable local SVG files plus license/provenance metadata.
---
...
Use this skill to search and download publication-friendly SVG icon assets into the current project. BioIcons is preferred for life-science concepts; Iconify is used for broad open-source icon coverage; Svg/icons CLI is optional for prompt-style recommendations and Pro workflows.
...
- Use `--provider auto` by default. It searches/downloads BioIcons first, then Iconify.
...
1. Start with a preset when the request matches one: `gene`, `dna`, `cell`, `mutation`, `virus`, `lab`, `antibody`, or `oncology`.
...
Always preserve `LICENSES.tsv`. BioIcons icons have per-icon licenses and authors. Iconify's framework license does not cover every icon; each icon set has its own license.
...
Use SVGs as building blocks for mechanism diagrams, graphical abstracts, posters, and slides. Do not present generic icons as exact biological structures. For data plots, statistics, Kaplan-Meier curves, forest plots, or full figure panels, use plotting or drawing workflows instead.
